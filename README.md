...
**Document ingest** — `document_extract` reads a PDF, Word `.docx`, Excel `.xlsx`, CSV or text file: an EOB, an ADR letter, an appeal determination, a remittance spreadsheet. Drag one onto the console and it is read at the door, so an unreadable file is known before a question is asked rather than after a round trip through the model. No new dependency — `.docx`/`.xlsx` are ZIP containers of XML and a ZIP reader over `node:zlib` is the whole integration.

PDF is the one that had to be built properly rather than approximated, because the naive version is not merely incomplete, it is **wrong in the digits**. A PDF stores positioned glyph ids against a subset font plus a per-font `ToUnicode` map; a reader that resolves one font renders a real remittance as `otal pai�� �������` while its decorative heading survives intact — the claim id, the check number and every dollar amount destroyed, the ornament kept. The common shortcut of treating unmapped ids as Latin-1 fails *silently* instead, returning digits that are wrong and look right. So fonts are resolved per page through the page's own resource dictionary, object streams are unpacked (most modern PDFs hide their font dictionaries there), and **a reading below 90% glyph coverage is refused** rather than returned with a caveat, since the holes fall wherever a font lacked a map and digits are as likely as letters. Reading order is rebuilt from the text-positioning operators, which is what separates a table row from the row beneath it — and separating a column break from a kerning advance is subtle enough that getting it wrong split `99211` into `9921` and `1` before the rule was written against a pen position rather than a font size.

Three things it refuses, each naming what to do instead: a **scanned PDF** (no text layer — there is no OCR here), an **image** (no OCR, and a text-only model cannot see it either, so both reasons are stated), and a **legacy `.doc`/`.xls`** (OLE compound format — guessing at BIFF records would produce numbers nobody could stand behind). An X12 envelope is routed to `era_parse_835` / `ack_parse_277ca` instead of being read as prose.
