...
export function buildRca(input: RcaInput): RcaReport {
  ...
  md.push(
    "---",
    "",
    "*Generated by Orion from the tool-call log and the claim lifecycle record in this database. It describes what this installation observed — anything that happened inside a clearinghouse or a payer system appears here only through what came back.*",
  );
  ...
  return {
    ...
    ticket: {
      title,
      severity,
      labels: ["orion", `cause:${primary}`, `tier:${ownership.tier}`, severity],
      fingerprint: fp,
