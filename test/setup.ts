...
// Without this, the suite reads ~/.aetheraclaw — the developer's OWN config and
...
//
// That failure mode is the dangerous shape. A CI runner has no ~/.aetheraclaw,
...
// somebody does. Pointing AETHERACLAW_HOME at an empty temp directory makes the
// suite depend on its fixtures and nothing else.
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aetheraclaw-test-home-"));
process.env.AETHERACLAW_HOME = dir;
